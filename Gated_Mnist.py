...
class GatedFeedbackNet(nn.Module):
    def __init__(self, input_dim=784, state_dim=128, n_blocks=2, feedback_scale=0.3):
        super().__init__()
        ...
        self.readout = nn.Linear(state_dim, 10)  # 10 classes

        self.fb_to_input = nn.Linear(state_dim, input_dim)
        self.gate_layer = nn.Linear(state_dim, input_dim)  # gate vector for feedback
...
def train_epoch(model, optimizer, max_iters=10, tol=1e-10):
    model.train()
    total_loss = 0.0
